    ...
    try {
      ...
      tmp.querySelectorAll("mjx-container").forEach((container) => {
        ...
        if (tex) {
          ...
          // Store raw TeX without delimiters for direct rendering
      ...
      tmp.querySelectorAll(".math-tex").forEach((span) => {
        ...
        if (scriptEl && scriptEl.textContent) {
          // Store raw LaTeX, no delimiters — dashboard renders directly
  ...
  function parseQuestionsFromHtml(rawHtml) {
    ...
    if (questions.length === 0) {
      ...
      if (optionLis.length > 0) {
        ...
        if (compEl) {
          passageHtmlStr = '<div class="tb-passage" style="margin-bottom:10px;font-size:12px;line-height:1.6;">' + cleanTestbookSpecifics(compEl.innerHTML) + '</div>';
