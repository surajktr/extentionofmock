...
if (historyId) {
    ...
    getFullDb().then((db) => {
        ...
        if (testTimerEl) testTimerEl.value = total;

        el.rangeDesc.textContent = `This subject has ${total} question${total !== 1 ? 's' : ''}. By default all are included.`;

        const updateTimer = () => {
            const f = parseInt(el.rangeFrom.value, 10);
            const t = parseInt(el.rangeTo.value, 10);
            if (!isNaN(f) && !isNaN(t) && t >= f) {
                if (testTimerEl) testTimerEl.value = (t - f + 1);
            }
        };
        el.rangeFrom.addEventListener('input', updateTimer);
        el.rangeTo.addEventListener('input', updateTimer);
...
function startTest() {
    ...
    const testTimerEl = document.getElementById('testTimer');
    let timerMins = testTimerEl ? parseFloat(testTimerEl.value) : questions.length;
    if (isNaN(timerMins) || timerMins <= 0) timerMins = questions.length;

    timerSeconds = Math.floor(timerMins * 60);
...
function renderCurrentQuestion() {
    ...
    q.options.forEach((opt, idx) => {
        ...
        if (showAnswer) label.style.pointerEvents = 'none';
        ...
        label.innerHTML =
            `<input type="radio" name="current_options" value="${idx}" ${isChecked ? 'checked' : ''}>` +
...
if (lightbox) {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.tagName === 'IMG' && (e.target.closest('.q-content') || e.target.closest('.notes-panel') || e.target.closest('.solution-panel'))) {
